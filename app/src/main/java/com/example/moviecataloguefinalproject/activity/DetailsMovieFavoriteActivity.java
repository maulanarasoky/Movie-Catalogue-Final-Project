...
public class DetailsMovieFavoriteActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ListDataFavoriteMovie movieFav = getIntent().getParcelableExtra(SEND_MOVIE_FAVORITE);
        ...
        assert movieFav != null;
        uriWithId = Uri.parse(DatabaseContract.MovieColumns.CONTENT_URI + "/" + movieFav.getId());

        if (uriWithId != null){
            Cursor cursor = getContentResolver().query(uriWithId, null, null, null, null);

            if (cursor != null){
                movieFav = MappingHelper.mapCursorToObjectMovie(cursor);
                cursor.close();
            }
        }
    }
                    ...
                    .setPositiveButton(getString(R.string.yes), new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
                            ...
                            if (result > 0) {
                                ...
                                sendUpdate(getApplicationContext());
                                finish();
                            } else {
